...
def main():
    ...
    for k in dimensions:
        ...
        print(f"  [Gabor + XPCA] Min-Max: {res_gab_xpca_mm['eer']*100:.2f}% | Softmax: {res_gab_xpca_sm['eer']*100:.2f}% | Residual: {res_gab_xpca_res['eer']*100:.2f}%")

    # 5. Output and format findings
    df_results = pd.DataFrame(records)
    df_results.to_csv("results/tables/gabor_xpca_comparison.csv", index=False)
    print("\nSaved comprehensive results to results/tables/gabor_xpca_comparison.csv")
